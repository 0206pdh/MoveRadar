...
| 신호 | 가중치 | 선행성 | 설정 근거 |
|------|--------|--------|-----------|
| 시세 | **0.40** | 1~2개월 선행 | 아파트 거래가 급변은 이사 수요의 가장 직접적인 시장 신호. 매수·매도 움직임이 실제 이사보다 먼저 나타나며, 동 단위 EMD 세분화로 국지적 수요 급변 포착 가능. 가장 신뢰도 높은 선행 지표로 최고 가중치 부여. |
| 전입인구 | **0.25** | 동행 (실이사) | 실제 이사가 완료된 시점의 확증 신호. 선행성은 낮지만 이사 수요의 직접 증거이므로 두 번째 가중치. 시세 신호와 함께 급등 시 경보 신뢰도를 크게 높임. |
| 통신 개통 | **0.20** | 0~1개월 선행 | 인터넷 신규 개통은 입주 직전·직후에 집중되는 이사 완료 증거. 전입인구보다 약간 선행하며, 통신사 실거래 데이터로 노이즈가 낮음. |
| 카드소비 | **0.15** | 1~3개월 선행 | 가전·가구·할인점 소비 급증은 이사 준비 단계 신호. 가장 이른 선행 신호이나 이사 외 소비 이벤트(명절·연말)와 혼동 가능성이 있어 낮은 가중치. 다른 신호와 복합될 때 변별력 확보. |

> 가중치 합계 = 1.00 (0.40 + 0.25 + 0.20 + 0.15)
...
```
PERCENTILE > 0.75  →  급등 경보  (상위 25% 이상 이상값)
PERCENTILE < 0.25  →  급락 경보  (하위 25% 이하 이상값)
```

**0.75 / 0.25 기준 선택 이유**:
- Snowflake Cortex Anomaly Detection은 탐지 시점 값의 역사적 분포 내 상대적 위치를 percentile로 반환
- `> 0.5` 수준은 평균 이상을 모두 포함해 노이즈가 과도하게 발생
- `> 0.90` 수준은 극단값만 탐지해 실제 수요 변화를 놓침
- **0.75 / 0.25** (상하위 25%)는 "비정상적으로 높거나 낮은 수준"을 실용적으로 구분하는 분위수로, 오탐과 누락 사이의 균형점

**급락 경보도 포함하는 이유**:  
시세 급락 = 매물 급증 = 이사 나가는 수요 급증. 전입인구 급락 = 공실 위험 지역. 급등과 급락 모두 이사 관련 마케팅 타이밍 신호로 활용.

---

### COMBINED_SCORE 산출 방식

각 신호의 percentile을 0~1 척도에서 "중앙(0.5)으로부터의 이탈 강도"로 변환 후 가중 합산.

```
COMBINED_SCORE =
    |price_pct  − 0.5| × 2 × 0.40   -- 시세
  + |pop_pct    − 0.5| × 2 × 0.25   -- 전입인구
  + |tc_pct     − 0.5| × 2 × 0.20   -- 통신 개통
  + |card_pct   − 0.5| × 2 × 0.15   -- 카드소비
```

- `|pct − 0.5| × 2` : 0.75 → 0.5, 0.25 → 0.5, 1.0 → 1.0 (이탈 강도를 0~1로 정규화)
- 단일 신호 최대 기여: 시세 0.40, 전입인구 0.25, 통신 0.20, 카드 0.15
- 4신호 모두 임계값 초과 시 이론적 최대 = 1.00

---

### 훈련/탐지 분리 전략

모델 과적합(leakage) 방지를 위해 훈련 기간과 탐지 기간을 겹치지 않게 설계.

| 신호 | 훈련 기간 | 탐지 기간 | 비고 |
|------|-----------|-----------|------|
| 시세 | ~2023 | 2023~ | 22개 구 2023 탐지, 3개 구는 데이터 시작이 늦어 2024+ |
| 전입인구 | 2021~2022 | 2023~ | no overlap |
| 통신 개통 | 2023 | 2024~ | V01 데이터 자체가 2023 시작, no overlap |
| 카드소비 | ~2023 | 2024~ | no overlap |
| TMAP | — | LLM 컨텍스트 전용 | 1개월치만 제공 → Anomaly Detection 불가 |
